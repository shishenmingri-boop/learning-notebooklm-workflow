...
| `learning-output/2026-07-01-pinterest-niche-reselection/` | ニッチ再選定調査パッケージ（詳細は13章） |

---

## 13. ニッチ再選定（2026-07-01）

「収納」ニッチはまだ実運用がdry-run段階（実績データなし）であることを踏まえ、ニッチ自体をゼロベースで再評価する調査を実施した。詳細な調査パッケージは `learning-output/2026-07-01-pinterest-niche-reselection/` を参照。

### 最終推奨

**「収納ニッチを完全に手放すのではなく、育児×収納への隣接拡張を次の一手とし、ペット用品／ファミリーキャンプ・アウトドアは中期的な第二ニッチ候補として保留する」**

### 根拠の要点

- 8軸スコアリング（保存性・検索需要・アフィリ料率・note相性・競合の少なさ・Flux相性・法規制リスク低さ・資産転用）で7候補ニッチを比較した結果、**収納・インテリア（現行）が31点で1位**、**ベビー・キッズ育児収納が30点で2位**（僅差）。
- 収納の最大の弱点は**アフィリ料率が最下位クラス**（楽天・Amazonともに3%。ペット/美容/ダイエット系は5%）であり、かつPinterestの「AI生成コンテンツを少なく表示」機能の対象カテゴリ（美容・アート・ファッション・**インテリア**）に含まれる点がリスクとして残る。
- 「育児×収納」への隣接拡張は、既存の収納ノウハウ・Fluxプロンプト・note比較記事フォーマットを80%以上転用できるうえ、料率が3%→4%に改善し、note公式データ（2026年1月、約30万件分析）でも「育児・教育」が急上昇カテゴリに入っている。
- ペット用品・ファミリーキャンプ・アウトドアは料率・保存性・検索需要で収納より優位（28点・同点2位グループ）だが、ゼロからのキーワード設計・プロンプト構築コストが高く、ペット用品はFlux画像生成との相性（no-people/no-face制約）が課題。中期的な第二ニッチ候補として保留する。
- プチプラ美容・宅トレ/ダイエット系は料率は良いが、競合密度（大手参入済みの激戦区）と法規制リスク（薬機法・景品表示法）が高く、現時点では推奨しない（20点）。
- NotebookLM Deep Researchにより、Pinterestの2026年AI画像抑制機能の適用範囲拡大の実態、およびnote.comにおける「育児・教育」×「収納・暮らし」の複合ジャンル運用の実務知見を補強調査済み（ノート内にレポート2件を格納）。

### 次アクション（本タスクのスコープ外・別タスクで実施）

ピボット（育児×収納への拡張）を実行する場合、以下は本調査タスクでは行っておらず、次の実装タスクとして別途依頼が必要:

1. `content-queue/keywords.csv` への「育児×収納」関連キーワードの追加・再生成
2. Fluxプロンプトテンプレートの育児収納向け調整（子どもの顔を出さない構図の徹底）
3. note記事構成・比較軸への「育児×収納」カテゴリの追加
4. 既存の収納ニッチ運用（`content-queue/post-result.md` 等）は変更せず、並行運用または段階移行を検討

### 関連資料

- 調査パッケージ全体: `learning-output/2026-07-01-pinterest-niche-reselection/`
- 比較表・スコアリング詳細: `learning-output/2026-07-01-pinterest-niche-reselection/02_deep_dive.md`
- NotebookLMノート: https://notebooklm.google.com/notebook/829dccc7-dd26-4520-bec2-1259a60cec1d
- NotebookLM投入結果: `learning-output/2026-07-01-pinterest-niche-reselection/notebooklm-upload-result.md`
